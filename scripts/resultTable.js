...
function fetchData() {
    return fetch('skylander.json')
        .then(response => response.json())
        .then(data => {
            const elemDict = data.elements
            const dataDictionary = data.data
            const keysDictionary = data.keys;
            elementList = elemDict
...
function guessTable(guess, guessList) {
    ...
    const guessElement = guessRow.insertCell(1);
    if (guessDetails["element"] == answerDetails["element"]){
        statStr =statStr + "🟩" 
        guessElement.id = "resTrue"
    } else{
        guessElement.id = "resFalse"
        statStr =statStr + "🟥"
    }
    ...
    const attr = ["gender", "species", "world", "role", "appears"]
    
    for (j = 0; j < 5; j++){
        statChar = guessCell(j+2, attr[j], guessRow, guessDetails)
    ...
    elemIcon.src=elementList[guessDetails.element.toLowerCase()]
    elemIcon.classList.add("resultIcon")
    guessElement.append(elemIcon)
...
function copyStats() {
    stats = statArray
    var copyStr = "I found Skylandle #" + today +" in " + stats.length +   " guesses!\n"
    for (i = 0; i < stats.length; i++){
        copyStr += stats[i]+ "\n"
    }
    copyStr += "\nPlay on https://skylandle.com"
    navigator.clipboard
      .writeText(copyStr)
